...
def fetch_data(numStages, numSubperiods, numSubterms, epsilon = 0):
    ...
    sigma_d_2023_electricity_demand = daily_std_2023.iloc[:, 0].values
    sigma_c_2023_electricity_demand = cluster_std_2023.iloc[:, 0].values
    combined_std_2023_electricity_demand = (sigma_d_2023_electricity_demand * sigma_c_2023_electricity_demand)[24:]
    electricity_demand_2023 = [x + epsilon * combined_std_2023_electricity_demand[i] for i, x in enumerate(electricity_demand_2023[24:])]

    daily_std_2024, cluster_std_2024 = _read_std_data('2024 Hourly Electricity Demand')
    sigma_d_2024_electricity_demand = daily_std_2024.iloc[:, 0].values
    sigma_c_2024_electricity_demand = cluster_std_2024.iloc[:, 0].values
    combined_std_2024_electricity_demand = (sigma_d_2024_electricity_demand * sigma_c_2024_electricity_demand)[:(8760-24)]
    electricity_demand_2024 = [x + epsilon * combined_std_2024_electricity_demand[i] for i, x in enumerate(electricity_demand_2024[:(8760-24)])]

    base_electricity_demand = [(val_2023 + val_2024) / 2 for val_2023, val_2024 in zip(electricity_demand_2023, electricity_demand_2024)]
    base_electricity_demand_std = [(s1 + s2) / 2 for s1, s2 in zip(combined_std_2023_electricity_demand, combined_std_2024_electricity_demand)]
    ...
    daily_std_2024_heat_demand, cluster_std_2024_heat_demand = _read_std_data('2024 Hourly Heat Demand')
    sigma_d_2024_heat_demand = daily_std_2024_heat_demand.iloc[:, 0].values
    sigma_c_2024_heat_demand = cluster_std_2024_heat_demand.iloc[:, 0].values
    combined_std_2024_heat_demand = (sigma_d_2024_heat_demand * sigma_c_2024_heat_demand)[:(8760-24)]
    base_heat_demand = [x + epsilon * combined_std_2024_heat_demand[i] for i, x in enumerate(heat_demand_2024[:(8760-24)])]
    base_heat_demand_std = combined_std_2024_heat_demand.tolist()
    ...
    combined_std_solar = daily_std_solar.values * cluster_std_solar.values
    base_solar_periodic_generation_std = [row[:(8760-24)] for row in combined_std_solar.T.tolist()]
    ...
    combined_std_wind = daily_std_wind.values * cluster_std_wind.values
    base_wind_periodic_generation_std = [row[:(8760-24)] for row in combined_std_wind.T.tolist()]
    ...
    combined_std_parabolic_trough = daily_std_pt.values * cluster_std_pt.values
    base_parabolic_trough_periodic_generation_std = [row[:(8760-24)] for row in combined_std_parabolic_trough.T.tolist()]
    ...
    combined_std_heat_pump = daily_std_hp.values * cluster_std_hp.values
    base_heat_pump_cop_std = [row[:(8760-24)] for row in combined_std_heat_pump.T.tolist()]
    ...
    electricity_demand_std = clustering_n_consecutive_data_points(base_electricity_demand_std, int((8760-24)/numSubterms))
    heat_demand_std = clustering_n_consecutive_data_points(base_heat_demand_std, int((8760-24)/numSubterms))
    solar_periodic_generation_std = [clustering_n_consecutive_data_points(v, int((8760-24)/numSubterms)) for v in base_solar_periodic_generation_std]
    wind_periodic_generation_std = [clustering_n_consecutive_data_points(v, int((8760-24)/numSubterms)) for v in base_wind_periodic_generation_std]
    parabolic_trough_periodic_generation_std = [clustering_n_consecutive_data_points(v, int((8760-24)/numSubterms)) for v in base_parabolic_trough_periodic_generation_std]
    heat_pump_cop_std = [clustering_n_consecutive_data_points_cop(v, int((8760-24)/numSubterms)) for v in base_heat_pump_cop_std]
