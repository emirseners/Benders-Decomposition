...
class ScenarioNodeMSSP:
    ...
    def AddVariables(self, model, continuous_flag):
        ...
        if self.id != 0:
            ...
            self.e_Carrying = model.addVars([(t, self.numSubterms) for t in self.stageSubperiods], vtype=GRB.CONTINUOUS, lb=0, ub=0, name="electricitycarry_"+str(self.id)) # inventory carriage amount
            self.h_Carrying = model.addVars([(t, self.numSubterms) for t in self.stageSubperiods], vtype=GRB.CONTINUOUS, lb=0, ub=0, name="heatcarry_"+str(self.id)) # inventory carriage amount
    ...
    def AddObjectiveCoefficients(self, electricity_purchasing_cost, heat_purchasing_cost, discount_factor):
        ...
        for t in self.stageSubperiods:
            for p in self.stageSubterms:
                self.e_Purchase[t,p].Obj = 0.001 * self.probability * electricity_purchasing_cost[t] * (discount_factor**(t))
                self.h_Purchase[t,p].Obj = 0.001 * self.probability * heat_purchasing_cost[t] * (discount_factor**(t))
